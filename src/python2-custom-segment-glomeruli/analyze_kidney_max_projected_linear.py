...
def segment_layer(filename, params):
	...
	sizefactor = 10.
	small = ndimage.interpolation.zoom(img, 1./sizefactor, order = 1, prefilter=False)	#scale the image to a smaller size
	...
	kmask = ndimage.interpolation.zoom(kmask, sizefactor, order = 1, prefilter=False)	#scale back to normal size
...
def label_cells(folder, params):
	...
	for i in range(len(files)):		
		...
		if len(labels) > limsize:
			...
			skimage.io.imsave(
				params.inputfolder +  '../segmented/labels/glomeruli/' + folder + fname[:-4] + '.tif',
				(np.array([lb, ind[0], ind[1]]) > 0).astype(np.uint8) * 255)
